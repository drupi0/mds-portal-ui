...
export class FormLoaderComponent implements OnInit {
  ...
  saveSort(sortColumns: string[]) {
    const cols = sortColumns.filter(
      (key) => !key.startsWith('asc') && !key.startsWith('desc')
    );

    if (
      JSON.stringify(cols) === JSON.stringify(this.pagination.sortKeys) &&
      sortColumns.find((key) => key === this.pagination.sortOrder)
    ) {
      return;
    }
      ...
      this.api
        ...
        .subscribe((searchResult: Pagination<PatientRecordModel>) => {
          ...
          this.searchPagination = {
            ...this.searchPagination,
            offset:
              this.searchPagination.currentPage * this.searchPagination.size,
  ...
  loadRecords() {
    ...
    this.api
      ...
      .subscribe((data: Pagination<PatientRecordModel>) => {
